// Vendored from radix-ui/website (https://github.com/radix-ui/website),
// MIT License, Copyright (c) 2022 WorkOS. Generates a full 12-step Radix
// color scale (plus alpha/contrast/surface variants) from a single hex by
// interpolating between the hand-tuned Radix scales in OKLCH.
// Local changes: inlined the type/scale-name definitions that the website
// imported from its own `types` module; RadixColors indexed via a cast.
import * as RadixColorsModule from "@radix-ui/colors";
import BezierEasing from "bezier-easing";
import Color from "colorjs.io";

const RadixColors = RadixColorsModule as unknown as Record<
  string,
...
// Kept from upstream for parity even though the light/dark paths we use only
// transpose the progression start.
